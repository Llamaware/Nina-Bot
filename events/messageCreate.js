// Message commands are disabled for now
...
module.exports = async (client, message) => {
	...
	// let now = Date.now();
	// let timeStamp = client.cooldowns.get(command.data.name) || new Collection();
	...
	// 	let cool = new EmbedBuilder()
	// 		.setDescription(`❌ Please wait ${(estimated / 1000).toFixed()}s more before reusing the ${command.data.name} command.`)
	...
	// client.cooldowns.set(command.data.name, timeStamp);
	// try {
	...
	// } finally {
	// 	client.logger.log(`> ID : ${message.author.id} | User : ${message.author.tag} | command | ${command.data.name}`, "info");
	// }
};
